...
time_t tstamp_created_at=0;
static uint32_t l_tstamp=0, r_tstamp=0;
static uint16_t mtu=0;

void get_postoptions(void) {
	uint32_t p_tstamp=0, t_tstamp=0;

	p_tstamp=l_tstamp + (tstamp_created_at - time(NULL));
	...
	switch (s->ss->fingerprint) {
		case 0: /* cisco ios */
			/* XXX shrug, i forgot to get this info from that cisco */
		...
		case 6: /*
			 * heh, linux looks identical to freebsd here, so i guess
			 * both freebsd and linux took that code from openserver...
			 */
		case 7: /* no comment */
		case 4: /* FreeBSD 5.1 */
			/* <nop,nop,timestamp XXXX201 XXXX256> */
			s->ss->posttcpoptions_len=2;

			/* N N ( size 2 ) */
			s->ss->posttcpoptions[0]=0x01; s->ss->posttcpoptions[1]=0x01;

			break; /* ehehe i guess it does piss of the host if we screw this up */
			/* T ( size 10 ) */
			s->ss->posttcpoptions[2]=0x08; s->ss->posttcpoptions[3]=0x0a;
			memcpy(s->ss->posttcpoptions + 4, &p_tstamp, sizeof(p_tstamp));
			/*
			 * oh whoops, we dont have this info XXX (yet) hopefully if we keep it zero
			 * our new friend wont have a brain freeze
			 */
			memcpy(s->ss->posttcpoptions + 8, &t_tstamp, sizeof(t_tstamp));
			break;

		case 5: /* nmap */
			/* ok dont care anymore, does this work i wonder? ;] */
		default:
			break;
	}
...
void init_packet(void) {
	...
	l_tstamp=(uint32_t)arc4random();
	r_tstamp=0;
	time(&tstamp_created_at);

	switch (s->ss->fingerprint) {
		case 0:
			/* Cisco IOS 12.1 on a 2600 router type device, from tcpdump */
			if (s->ss->ttl == 0) s->ss->ttl=255;
			s->ss->ip_off=0;
			...
			if (s->ss->mode == MODE_TCPSCAN) {
				...
				mtu=htons(s->vi->mtu - 40);
				memcpy(s->ss->tcpoptions + 2, &mtu, sizeof(mtu));
			}
			break;
		case 1:
			if (s->ss->ttl == 0) s->ss->ttl=64;
			if (s->ss->ip_off == 0) s->ss->ip_off=IP_DF;

			/*
			 * openbsd 3.0-3.4 from the p0f fp file
			 * i dont accually run openbsd, its too secure for me...
			 */
			if (s->ss->mode == MODE_TCPSCAN) {
		...
		case 3: /* p0f sendsyn (aprox) */
			if (s->ss->ttl == 0) s->ss->ttl=255;
			if (s->ss->ip_off == 0) s->ss->ip_off=0;
...
*/
			...
			if (s->ss->mode == MODE_TCPSCAN) {
				if (s->ss->tos == 0) s->ss->tos=0x10; /* its telnet! */
...
*/
			if (s->ss->ttl == 0) s->ss->ttl=61; /* just to break it up a little ;] */
			if (s->ss->ip_off == 0) s->ss->ip_off=0;
...
*/
			...
			if (s->ss->mode == MODE_TCPSCAN) {
				s->ss->tcpoptions_len=20;
				mtu=htons(s->vi->mtu - 64);
				s->ss->window_size=(s->vi->mtu - 64) * 4;
				/* MSS size 4 */
				s->ss->tcpoptions[0]=0x02; s->ss->tcpoptions[1]=0x04;
				memcpy(s->ss->tcpoptions + 2, &mtu, sizeof(mtu));
				/* S ( size 2 ) */
		...
		case 7: /* some crazy stuff i just made up */
			...
			if (s->ss->mode == MODE_TCPSCAN) {
				...
				s->ss->window_size=(s->vi->mtu - 32) * 8;
			...
			break;
		default:
			MSG(M_ERR, "Unknown fingerprint `%d', defaulting to 0", s->ss->fingerprint);
			s->ss->fingerprint=0;
			init_packet();
			break;
	}
