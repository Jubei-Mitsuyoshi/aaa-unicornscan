...
#define DEBUG(fmt, args...) display_builtin(M_DBG1, __FILE__, __LINE__, fmt, ## args);
#define FATAL(fmt, args...) do { display_builtin(M_ERR, __FILE__, __LINE__, fmt, ## args); return; } while (0)
...
void load_config(void) {
  ...
  if (ack_mode) {
    cf=CONFIG_DIR "/" SYNACK_DB;
  }
  else {
    cf=CONFIG_DIR "/" RST_DB;
  }
  c = fopen(cf ,"r");
...
continue_fuzzy:    
    ...
    if (!no_known) {
      ...
      if (tos) {
        if (tos_desc) outbuf_add("[%s] ",tos_desc); else outbuf_add("[tos %d] ",tos);
      }
